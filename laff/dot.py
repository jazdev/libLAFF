...
def dot(x, y, alpha=0):
    """
    Compute alpha = x' * y, storing the result in alpha and returning 
    alpha.
    ...
    assert type(x) is matrix and len(x.shape) is 2, "laff.dot: vector x must be a 2D numpy.matrix"

    assert type(y) is matrix and len(y.shape) is 2, "laff.dot: vector y must be a 2D numpy.matrix"
                
    if(type(alpha) is matrix): m_alpha, n_alpha = alpha.shape
        
    assert isinstance(alpha,(int,float,complex)) or (m_alpha is 1 and n_alpha is 1), "laff.scal: alpha must be a 1 x 1 matrix"
